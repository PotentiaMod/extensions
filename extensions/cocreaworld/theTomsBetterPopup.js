...
//↙点这里收起
{
    ...
    const plusButton =
        ...
        'FjLTEuMTA0IDAtMiAuODk2LTIgMnMuODk2IDIgMiAybDQuMDcxLS4wNzEtLjA3MSA0LjA3AMW' +
    ...
    function initExpandableBlock(runtime, blockDefinition, dynamicArgInfo) {
        ...
        blockDefinition.init = function () {
            ...
            if (!this.getInput) return; // 避免协作报错，奇怪的bug
    ...
    function initExpandableBlocks(extension, plusImage = rightArrow, minusImage = leftArrow) {
        // 创建按钮
        const runtime = extension.runtime || extension._runtime;

        // 这一句是为了防止测试时重新加载扩展，可以删掉
        // if (runtime.__EXPANDABLE_REG) return;
        // runtime.__EXPANDABLE_REG = true;
...
(function(Scratch) {
    ...
    const EXTENSION_ID = "theTomsBetterPopup";
    ...
    // 广播事件绑定: {open: {type: broadcast}, close: {type: broadcast}}
    let _broadcastBindings = {open: {}, close: {}};
    ...
    if (Scratch.translate && Scratch.translate.setup) {
        Scratch.translate.setup({
            zh: {
                ...
                'block.pythonPopup': '用Python文件 [FILE] 生成弹窗并返回',
                'tooltip.pythonPopup': '从Gandi文件编辑器加载Python脚本，解析print/input/select等语句生成弹窗，返回用户输入结果(JSON)',
                'menu.pythonFile': 'Python文件',
                'block.popupFilePopup': '用Popup文件 [FILE] 生成弹窗并返回',
                'tooltip.popupFilePopup': '从Gandi文件编辑器加载.popup文件，解析交互标签生成弹窗，返回用户输入结果(JSON)',
                'menu.popupFile': 'Popup文件',
                'block.downloadFile': '下载文件 文件名[FILENAME] 内容[CONTENT] 类型[TYPE]',
                'block.uploadAndRead': '上传类型 [EXT] 的文件并读取为 [FORMAT]',
                'block.getFileInfo': '获取文件 [FILE] 的 [PROPERTY]',
                'tooltip.getFileInfo': '获取Gandi文件编辑器中指定文件的属性信息',
                'menu.fileProperty.all': '全部属性(JSON)',
                'menu.fileProperty.contentType': 'contentType',
                'menu.fileProperty.name': 'name',
                'menu.fileProperty.runtimeFormat': 'runtimeFormat',
                'menu.fileProperty.immutable': 'immutable',
                'menu.fileProperty.content': '内容',
                'block.showStageMenu': '显示菜单在舞台的 x:[X] y:[Y], 内容为 [ITEMS] 并返回',
                'block.generateMenuItems': '生成菜单内容',
                'tooltip.generateMenuItems': '动态生成菜单JSON内容，点击▲添加菜单项',
                'block.generateSubMenu': '生成子菜单内容 名称[NAME] 子菜单内容[CHILDREN]',
                'tooltip.generateSubMenu': '生成子菜单JSON对象，包含名称和子项内容',
                'block.bindBroadcast': '绑定广播事件 [BROADCAST] 于弹窗类型 [TYPE] [EVENT]',
                ...
                'file.noFile': '📁 没有文件',
                'file.noPythonFile': '📁 没有Python文件',
                'file.noPopupFile': '📁 没有Popup文件',
                ...
                'default.contextMenu.submenu': '子菜单',
            ...
            en: {
                ...
                'block.pythonPopup': 'generate popup from Python file [FILE] and return',
                'tooltip.pythonPopup': 'Load Python script from Gandi file editor, parse print/input/select statements to generate popup, return user input as JSON',
                'menu.pythonFile': 'Python File',
                'block.popupFilePopup': 'generate popup from Popup file [FILE] and return',
                'tooltip.popupFilePopup': 'Load .popup file from Gandi file editor, parse interactive tags to generate popup, return user input as JSON',
                'menu.popupFile': 'Popup File',
                ...
                'block.getFileInfo': 'get file [FILE] property [PROPERTY]',
                'tooltip.getFileInfo': 'Get property info of specified file in Gandi file editor',
                'menu.fileProperty.all': 'All Properties(JSON)',
                'menu.fileProperty.contentType': 'contentType',
                'menu.fileProperty.name': 'name',
                'menu.fileProperty.runtimeFormat': 'runtimeFormat',
                'menu.fileProperty.immutable': 'immutable',
                'menu.fileProperty.content': 'Content',
                'block.showStageMenu': 'Show Menu At Stage x:[X] y:[Y] Items[ITEMS] And Return',
                'block.generateMenuItems': 'Generate Menu Content',
                'tooltip.generateMenuItems': 'Dynamically generate menu JSON content, click ▲ to add menu items',
                'block.generateSubMenu': 'Generate SubMenu Name[NAME] Children[CHILDREN]',
                'tooltip.generateSubMenu': 'Generate submenu JSON object with name and children content',
                'block.bindBroadcast': 'Bind Broadcast [BROADCAST] To Type [TYPE] [EVENT]',
                ...
                'file.noFile': '📁 No File',
                'file.noPythonFile': '📁 No Python File',
                'file.noPopupFile': '📁 No Popup File',
                ...
                'default.contextMenu.submenu': 'Submenu',
    ...
    function translate(key) {
        if (Scratch.translate) {
            return Scratch.translate({id: key});
        }
        return key;
    }
    ...
    class BetterPopup {
        constructor(runtime) {
            ...
            if (typeof initExpandableBlocks === "function") initExpandableBlocks(this);
            // Patch ScratchBlocks to support XML blocks
            this._patchScratchBlocks();
            ...
            // 注册 popup 文件类型到 Gandi 文件编辑器
            this._registerPopupFileType();

            // 注册 Monaco 编辑器 popup 语言高亮
            this._registerPopupLanguage();
        }
        ...
        // Patch ScratchBlocks to support XML blocks (making shadow blocks draggable)
        _patchScratchBlocks() {
            if (this.runtime._betterPopupBlocksPatched) return;
            this.runtime._betterPopupBlocksPatched = true;

            const Blockly = this.runtime.scratchBlocks || window.ScratchBlocks;
            if (!Blockly) return;

            // Patch isShadowArgumentReporter to recognize our shadow blocks
            const isar = Blockly.scratchBlocksUtils.isShadowArgumentReporter.bind(Blockly.scratchBlocksUtils);
            Blockly.scratchBlocksUtils.isShadowArgumentReporter = b => {
                const shadowTypes = [
                    'theTomsBetterPopup_generateMenuItems',
                    'theTomsBetterPopup_generateSubMenu'
                ];
                return shadowTypes.includes(b.type) && b.isShadow() ? true : isar(b);
            };

            // Patch _convertBlockForScratchBlocks to handle XML blocks
            const cfbsb = this.runtime._convertBlockForScratchBlocks.bind(this.runtime);
            this.runtime._convertBlockForScratchBlocks = (b, c) => {
                if (c.id === EXTENSION_ID) {
                    if (b.blockType === 'xml') return { info: b, xml: b.xml };
                    return cfbsb(b, c);
                } else return cfbsb(b, c);
            };
        }
        ...
        getInfo() {
            return {
                ...
                blocks: [
                    ...
                    {
                        opcode: 'generateMenuItems',
                        blockType: BlockType.REPORTER,
                        text: translate('block.generateMenuItems'),
                        tooltip: translate('tooltip.generateMenuItems'),
                        arguments: {},
                        disableMonitor: true,
                        dynamicArgsInfo: {
                            dynamicArgTypes: ['s'],
                            ...
                            defaultValues: (i) => {
                                ...
                                if (i % 2 === 0) {
                                    const types = ['Button', 'Text', 'Line', 'SubMenu'];
                                    return types[idx % 4];
                                }
                                if(idx % 4 === 3) { return '{"value": "子菜单", "children": []}' } else { return "value" + idx; }
                            }
                        },
                    },
                    {
                        opcode: 'generateSubMenu',
                        blockType: BlockType.REPORTER,
                        text: translate('block.generateSubMenu'),
                        tooltip: translate('tooltip.generateSubMenu'),
                        hideFromPalette: true,
                        arguments: {
                            NAME: {
                                type: ArgumentType.STRING,
                                defaultValue: translate('default.contextMenu.submenu')
                            },
                            CHILDREN: {
                                type: ArgumentType.STRING,
                                defaultValue: '[]'
                    ...
                    {
                        blockType: 'xml',
                        xml: '<block type="theTomsBetterPopup_generateSubMenu">'
                            + '<value name="CHILDREN"><shadow type="theTomsBetterPopup_generateMenuItems"/></value>'
                            + '<value name="NAME"><shadow type="text"><field name="TEXT">子菜单</field></shadow></value>'
                        + '</block>'
                    },
                    '---',
                    ...
                    },
                    {
                        opcode: 'getFileInfo',
                        blockType: BlockType.REPORTER,
                        text: translate('block.getFileInfo'),
                        tooltip: translate('tooltip.getFileInfo'),
                        arguments: {
                            FILE: {
                                type: ArgumentType.STRING,
                                menu: 'gandiFileMenu',
                ...
                menus: {
                    ...
                    pythonFileMenu: {
                        ...
                        items: '_getPythonFileList'
                    },
                    popupFileMenu: {
                        acceptReporters: true,
                        items: '_getPopupFileList'
                    },
                    fileExtMenu: {
                        acceptReporters: true,
                        items: [
                    ...
                    filePropertyMenu: {
                        acceptReporters: false,
                        items: [
                            { text: translate('menu.fileProperty.all'), value: 'all' },
                            { text: translate('menu.fileProperty.contentType'), value: 'contentType' },
                            { text: translate('menu.fileProperty.name'), value: 'name' },
                            { text: translate('menu.fileProperty.runtimeFormat'), value: 'runtimeFormat' },
                            { text: translate('menu.fileProperty.immutable'), value: 'immutable' },
                            { text: translate('menu.fileProperty.content'), value: 'content' }
                        ]
                    },
                    gandiFileMenu: {
                        items: '_getGandiFileList'
                    },
                    languageMenu: {
                        acceptReporters: true,
                        items: [
        ...
        showConfirmAndWait(args, util) {
            // 始终等待模式：返回Promise，等待弹窗关闭后返回确认结果
            return this._showModal({
                type: Scratch.Cast.toString(args.TYPE),
                title: Scratch.Cast.toString(args.TITLE),
                content: Scratch.Cast.toString(args.CONTENT),
        ...
        showOptionsAndWait(args, util) {
            ...
            return this._showOptionsModal({
                ...
                // 多选模式返回JSON数组
                if (mode === 'checkbox' && Array.isArray(this._lastOptionsResult)) {
        ...
        showPanel(args, util) {
            ...
            if (waitMode === 'wait') {
                return this._showCustomPanel(panelConfig).then(() => {
        ...
        showFileImport(args, util) {
            ...
            if (waitMode === 'wait') {
                return this._showFileImportPopup(popupConfig).then(() => {
                    return JSON.stringify(_lastFileImportResult);
                });
            } else {
        ...
        bindBroadcast(args) {
            ...
            console.log('[绑定调试] BROADCAST:', broadcast);
            console.log('[绑定调试] TYPE:', type);
            console.log('[绑定调试] EVENT:', event);
            console.log('[绑定调试] args.EVENT原始值:', args.EVENT);
            
            _broadcastBindings[event][type] = broadcast;
            console.log('[绑定调试] 已保存到: [' + event + '][' + type + '] = ' + broadcast);
            console.log('[绑定调试] 当前_broadcastBindings:', JSON.stringify(_broadcastBindings));
        }
        
        // 触发绑定的广播
        _triggerBroadcasts(popupType, event) {
            console.log('[广播调试] _triggerBroadcasts called:', popupType, event);
            console.log('[广播调试] _broadcastBindings:', JSON.stringify(_broadcastBindings));
            console.log('[广播调试] _savedRuntime:', _savedRuntime ? '存在' : '不存在');
            
            const runtimeToUse = _savedRuntime || this.runtime;
            if (!runtimeToUse) {
                console.error('[广播调试] ✗ 没有可用的runtime！');
                return;
            }
            
            // 触发特定类型的广播
            if (_broadcastBindings[event] && _broadcastBindings[event][popupType]) {
                const bc = _broadcastBindings[event][popupType];
                console.log('[广播调试] ✓ 触发广播: ' + bc + ' [' + popupType + ' ' + event + ']');
                
                try {
                    // 方法1：使用startHats
                    runtimeToUse.startHats('event_whenbroadcastreceived', {
                        fields: {
                            BROADCAST_OPTION: {
                                id: bc,
                                name: bc
                    ...
                    console.log('[广播调试] ✓ startHats调用成功');
                    
                    // 方法2：直接遍历所有target（备用方案）
                    if (runtimeToUse.targets) {
                        console.log('[广播调试] 遍历targets数量:', runtimeToUse.targets.length);
                        runtimeToUse.targets.forEach(target => {
                            if (target.blocks) {
                                // 查找所有event_whenbroadcastreceived积木
                                const blocks = target.blocks._blocks;
                                if (blocks) {
                                    Object.keys(blocks).forEach(blockId => {
                                        const block = blocks[blockId];
                                        if (block && block.opcode === 'event_whenbroadcastreceived') {
                                            const broadcastField = block.fields && block.fields.BROADCAST_OPTION;
                                            if (broadcastField && (broadcastField.value === bc || broadcastField.text === bc)) {
                                                console.log('[广播调试] 找到匹配的Hat积木:', blockId);
                                            }
                                        }
                                    });
                                }
                    ...
                    console.error('[广播调试] ✗ startHats调用失败:', e);
                }
            } else {
                console.log('[广播调试] 未找到绑定: [' + popupType + '] ' + event);
            }
            
            // 触发"所有"类型的广播
            if (popupType !== 'all' && _broadcastBindings[event] && _broadcastBindings[event]['all']) {
                const bc = _broadcastBindings[event]['all'];
                console.log('[广播调试] ✓ 触发广播: ' + bc + ' [all ' + event + ']');
                ...
                try {
                    ...
                    console.log('[广播调试] ✓ startHats调用成功(all)');
                } catch (e) {
                    console.error('[广播调试] ✗ startHats调用失败(all):', e);
                }
            }
        }
        ...
        _triggerButtonClicked(buttonValue) {
            console.log('Triggering hat for button:', buttonValue);
            console.log('_currentTarget:', this._currentTarget);
            
            // 存储到target上
            let targetToUse = this._currentTarget;
            
            // 如果_currentTarget不存在，尝试从runtime获取editingTarget
            if (!targetToUse && this.runtime && this.runtime.editingTarget) {
                targetToUse = this.runtime.editingTarget;
                console.log('Using runtime.editingTarget as fallback');
            }
            
            if (targetToUse) {
                targetToUse._lastButtonClicked = {
                    value: buttonValue
                };
                console.log('✓ Stored _lastButtonClicked on target');
            } else {
                console.error('✗ No target available for hat trigger!');
            }
            
            console.log(translate('panel.buttonClicked') + ': ' + buttonValue);
            
            // 使用startHatsWithParams触发Hat积木（CCW方式）
            const self = this;
            if (self.runtime && typeof self.runtime.startHatsWithParams === 'function') {
                console.log('Calling startHatsWithParams...');
                if (isCCW) {
                    ...
                    console.log('✓ startHatsWithParams called');
                }
            } else if (self.runtime) {
                // 标准Scratch的fallback
                console.log('Calling standard startHats...');
                self.runtime.startHats(`${EXTENSION_ID}_whenButtonClicked`);
            } else {
                console.error('✗ No runtime available!');
            }
        }
        ...
        _showFileImportPopup(popupConfig) {
            ...
            return this._showModal({
                ...
                if (result) {
                    // 从 window._fileImportResult 获取结果
        ...
        closePopup() {
            if (activePopup) {
                ...
                // 使用setTimeout异步触发广播，不阻塞关闭操作
                const self = this;
                setTimeout(() => {
                    self._triggerBroadcasts(popupType, 'close');
                }, 0);
        ...
        // 获取Gandi文件编辑器的所有文件列表（动态菜单）
        _getGandiFileList() {
            try {
                // 获取所有支持的文件类型
                const assetTypes = this.runtime?.gandi?._supportedAssetTypes;
                if (!assetTypes || assetTypes.length === 0) {
                    return [{ text: translate('file.noFile'), value: '' }];
                }
                // 获取所有类型的扩展名
                const extensions = assetTypes.map(t => t.runtimeFormat);
                // 获取所有文件
                const files = this.runtime?.getGandiAssetsFileList(extensions);
        ...
        // 获取文件属性
        getFileInfo(args, util) {
            const fileName = Scratch.Cast.toString(args.FILE);
            ...
            try {
                ...
                // 查找匹配的文件
                const fileInfo = files.find(f => f.fullName === fileName);
                if (!fileInfo) return '{}';
                ...
                if (property === 'all') {
                    return JSON.stringify({
                        ...
                        runtimeFormat: fileInfo.runtimeFormat || '',
                        immutable: fileInfo.immutable || false
                    });
                } else if (property === 'content') {
                    // 获取文件内容
                    const content = this.runtime?.getGandiAssetContent(fileName);
                    if (!content) return '';
                    // 尝试获取文本内容
                    if (typeof content === 'string') return content;
                    if (content.data) return content.data;
                    if (typeof content.encodeDataURI === 'function') return content.encodeDataURI();
                    return String(content);
                } else if (property in fileInfo) {
                    return String(fileInfo[property]);
                }
                return '';
            } catch (e) {
                console.warn('获取文件属性失败:', e);
                return '{}';
            }
        }

        // 获取Python文件列表（从Gandi文件编辑器）
        _getPythonFileList() {
            try {
                const files = this.runtime?.getGandiAssetsFileList(['py']);
                if (!files || files.length === 0) {
                    return [{ text: translate('file.noPythonFile'), value: '' }];
                }
                // 严格过滤，只保留 .py 文件
                const pyFiles = files.filter(f => f.fullName.toLowerCase().endsWith('.py'));
                if (pyFiles.length === 0) {
                    return [{ text: translate('file.noPythonFile'), value: '' }];
                }
        ...
        pythonPopup(args, util) {
            ...
            return new Promise((resolve) => {
                try {
                    const content = self.runtime?.getGandiAssetContent(fileName);
                    if (!content) {
                        console.warn('Python文件加载失败:', fileName);
                        self.logSystem.error('Python File "'+fileName+'" cannot be readed')
                        resolve('{}');
                        return;
                    }
                    ...
                    let pyCode = '';
                    try {
                        const dataUri = content.encodeDataURI();
                        const base64 = dataUri.split(',')[1];
                        pyCode = decodeURIComponent(escape(atob(base64)));
                    } catch (e) {
                        if (content.data && content.data.content) {
                            pyCode = content.data.content;
                        } else {
                            console.error('无法读取Python内容:', e);
                            self.logSystem.error('Python File "'+fileName+'" cannot be readed')
                            resolve('{}');
                            return;
                        }
                    ...
                    self.logSystem.error('Python Popup parse failed: ' + e.message);
                    resolve('{}');
                }
            });
        }
        ...
        // 注册 popup 文件类型到 Gandi
        _registerPopupFileType() {
            try {
                const assetTypes = this.runtime?.gandi?._supportedAssetTypes;
                if (!assetTypes) return;
                // 检查是否已注册
                if (assetTypes.findIndex(item => item.runtimeFormat === 'popup') !== -1) return;
                assetTypes.push({
                ...
                console.log('✅ Popup 文件类型已注册到 Gandi');
            } catch (e) {
                console.warn('⚠️ 注册 Popup 文件类型失败:', e);
            }
            console.log(this.runtime)
        }

        // 注册 Monaco 编辑器 popup 语言高亮
        _registerPopupLanguage() {
            try {
                ...
                // 语法高亮
                monaco.languages.setMonarchTokensProvider('popup', {
                    tokenizer: {
                        root: [
                            // HTML注释
                            [/<!--/, 'comment', '@comment'],
                            // 标签
                            [/<\/?(title|text|input|select|option|button|file|custom|slider|color|image)\b/, 'tag', '@tagAttrs'],
                            // 属性值中的字符串
                            [/"[^"]*"/, 'string'],
                            [/'[^']*'/, 'string'],
                            // 普通文本
                ...
                console.log('✅ Popup 语言高亮已注册');
            } catch (e) {
                console.warn('⚠️ 注册 Popup 语言高亮失败:', e);
            }
        }
        ...
        _getPopupFileList() {
            try {
                const files = this.runtime?.getGandiAssetsFileList(['popup']);
                if (!files || files.length === 0) {
                    return [{ text: translate('file.noPopupFile'), value: '' }];
                }
                const filtered = files.filter(f => f.fullName.toLowerCase().endsWith('.popup'));
        ...
        popupFilePopup(args, util) {
            ...
            return new Promise((resolve) => {
                try {
                    const content = self.runtime?.getGandiAssetContent(fileName);
                    if (!content) {
                        console.warn('Popup文件加载失败:', fileName);
                        self.logSystem.error('Popup File "' + fileName + '" cannot be readed');
                        resolve('{}');
                        return;
                    }
                    let popupCode = '';
                    try {
                        const dataUri = content.encodeDataURI();
                        const base64 = dataUri.split(',')[1];
                        popupCode = decodeURIComponent(escape(atob(base64)));
                    } catch (e) {
                        if (content.data && content.data.content) {
                            popupCode = content.data.content;
                        } else {
                            console.error('无法读取Popup内容:', e);
                            self.logSystem.error('Popup File "' + fileName + '" cannot be readed');
                            resolve('{}');
                            return;
                        }
                    ...
                    console.error('Popup文件执行失败:', e);
                    self.logSystem.error('Popup File parse failed: ' + e.message);
                    resolve('{}');
                }
            });
        }
        ...
        // 生成菜单内容（动态输入）
        generateMenuItems(args) {
            // 获取动态参数
            const items = [];
            for (let i = 1; ; i += 2) {
                const type = args['DYNAMIC_ARGS' + i];
                const value = args['DYNAMIC_ARGS' + (i + 1)];
                
                if (type === undefined) break;
                
                const typeStr = Scratch.Cast.toString(type);
                ...
                if (typeStr === 'Line') {
                    ...
                    // 子菜单：value可以是JSON字符串作为children，或只是菜单名
                    let children = [];
                    let menuName = valueStr;
                    
                    // 尝试解析value为JSON数组（作为children）
                    try {
                        const parsed = JSON.parse(valueStr);
                        if (typeof parsed === 'object' && !Array.isArray(parsed)) {
                            children = parsed.children;
                            menuName = parsed.value; // 默认名称
                        }
                    } catch (e) {
                        // 不是JSON，就当作菜单名
                    }
        ...
        // 生成子菜单内容
        generateSubMenu(args) {
            const name = Scratch.Cast.toString(args.NAME);
            const childrenStr = Scratch.Cast.toString(args.CHILDREN);
        ...
        _showModal(options) {
            ...
            setTimeout(() => {
                ...
                // 异步触发开启广播，不阻塞当前线程
                self._triggerBroadcasts(popupType, 'open');
            }, 100);
    ...
    if (isCCW) {
        window.tempExt = {
            Extension: BetterPopup,
            info: {
                name: "extensionName",
                description: "description",
                extensionId: EXTENSION_ID,
                iconURL: EXTENSION_PICTURE,
                insetIconURL: EXTENSION_ICON,
                featured: true,
                disabled: false,
                collaboratorList: [
                    {
                        collaborator: "༺ཌༀཉི 李 Tom ༃ༀད༻ @ CCW",
                        collaboratorURL: "https://www.ccw.site/student/63c28c9d058b205e558837ca",
                    }
                ]
            },
            l10n: {
                "zh-cn": {
                    "extensionName": "更好的弹窗",
                    "description": "美观实用的弹窗扩展"
                },
                en: {
                    "extensionName": "Better Popup",
                    "description": "Beautiful and practical popup extension"
                }
            }
        };
    } else {
