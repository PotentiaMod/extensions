(function (_Scratch) {
    ...
    class AdvancedDialog {
        ...
        _loadSettings() {
            try {
                const saved = localStorage.getItem('advancedDialogSettings');
                if (saved) {
        ...
        _saveSettings() {
            try {
                ...
                localStorage.setItem('advancedDialogSettings', JSON.stringify(settings));
            } catch (e) {
        ...
        _bindPanelEvents(panel, toggleBtn) {
            ...
            clearAllBtn.addEventListener('click', () => {
                if (confirm('确定要清除所有保存的数据吗？此操作不可恢复！')) {
                    localStorage.removeItem('advancedDialogSettings');
                    this._showSaveIndicator(panel);
                    this.showCornerToast({
                        TITLE: '数据已清除',
                        MESSAGE: '所有保存的设置已删除',
        ...
        // ----- getInfo（已移除 setCustomHtmlContent 和 showHtmlDialog）-----
        getInfo() {
            return {
                id: 'advancedDialog',
                color1: '#9C27B0',
        ...
        // ----- 公开弹窗方法（所有输入转义在 _createDialog 中完成）-----
        showAlert(args) {
